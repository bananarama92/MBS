import { logger, isArray } from "../../common";
...
export function getGroupNameKwargs(
    ...
    let isDefault: boolean | undefined = undefined;
    switch (defaults) {
        case "all":
            isDefault = true;
            break;
...
function validateOptions<T extends ExtendedWheelEvents.Events.Names>(
    ...
    if (!isArray(options.description) && typeof options.description !== "string" && description != null) {
        throw new TypeError(`${prefix}: Invalid "description" type: "${typeof options.label}"`);
...
export class WheelHookRegister implements _WheelHookRegister {
    ...
    run<T extends ExtendedWheelEvents.Events.Names> (
        ...
        allKwargs: Record<string, Record<string, WheelEvents.Kwargs.All>>,
    ): NonNullable<ExtendedWheelEvents.ListenerReturnType[T]>[] {
        return this[hookType].map(({ listener, hookName, conditional, registrationData }) => {
            const key = `${registrationData.name}-${hookType}-${hookName}`;
            const kwargs = allKwargs[key] ?? {};
            ...
            try {
                ...
                logger.error(`Failed to run ${key} listener`, error);
            }
            return result;
