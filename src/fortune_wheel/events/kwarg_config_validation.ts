...
function _validateNumber(
    ...
    if (typeof kwarg.default !== "number") {
        ...
        throw new Error(`${errPrefix}: Invalid kwarg "${name}.default" value: "${typeof (kwarg as any).default}"`);
    }
    if (typeof kwarg.min !== "number") {
        ...
        throw new Error(`${errPrefix}: Invalid kwarg "${name}.min" value: "${typeof (kwarg as any).min}"`);
    }
    if (typeof kwarg.max !== "number") {
        ...
        throw new Error(`${errPrefix}: Invalid kwarg "${name}.max" value: "${typeof (kwarg as any).max}"`);
    }
    if (kwarg.step != null) {
        if (typeof kwarg.step !== "number") {
            ...
            throw new Error(`${errPrefix}: Invalid kwarg "${name}.step" value: "${typeof (kwarg as any).step}"`);
        }
    }
...
export function validateKwargsConfig(
    ...
    for (const [name, kwarg] of Object.entries(kwargs)) {
        ...
        switch (kwarg.type) {
            case "number":
                _validateNumber(kwarg, errPrefix);
                break;
            case "text":
                _validateText(kwarg, errPrefix);
                break;
            case "checkbox":
                _validateCheckbox(kwarg, errPrefix);
                break;
            case "select":
            case "select-multiple":
                _validateSelect(kwarg, errPrefix);
                break;
            default:
