...
export function fortuneWheelEquip(
    ...
    const hookKwargs = Object.fromEntries(activeHooks.map(i => [i.id, i.kwargs]));
    ...
    for (const { Name, Group, Equip, NoEquip } of <(FWItem & { NoEquip?: boolean })[]>[...blockingItems, ...itemList]) {
        ...
        if (asset == null || group == null) {
            ...
            const validateEvent: Events.ValidateItemUnequip = {
                character,
                name,
                oldItem: oldItem,
            ...
            const equipChecks = wheelCallbacks.run("validateItemUnequip", validateEvent, hookKwargs, cache);
    ...
    for (const { Name, Group, Craft, Color, TypeRecord, Property } of itemList) {
        ...
        const validateEvent: Events.ValidateItemEquip = {
            character,
            name,
            oldItem,
            newAsset: asset,
        };
        const equipChecks = wheelCallbacks.run("validateItemEquip", validateEvent, hookKwargs, cache);
        if (equipChecks.length) {
            equipFailureRecord[`${Group}/${Name}`] = equipChecks;
            continue;
        }

        const beforeEquipEvent: Events.BeforeItemEquip = {
            character,
            name,
            oldItem,
            newAsset: asset,
        ...
        wheelCallbacks.run("beforeItemEquip", beforeEquipEvent, hookKwargs, cache);
        ...
        const colorOutput = wheelCallbacks.run("color", colorEvent, hookKwargs, cache);
        ...
        wheelCallbacks.run("typeRecord", typeEvent, hookKwargs, cache).forEach(t => {
            typeRecord ??= {};
        ...
        wheelCallbacks.run("property", propertyEvent, hookKwargs, cache).forEach(p => {
        ...
        if (asset.Group.IsItem() && !asset.IsLock && asset.Wear && asset.Enable) {
            ...
            const craftingOutput = wheelCallbacks.run("craft", craftingEvent, hookKwargs, cache);
        ...
        newItem.Difficulty += sum(wheelCallbacks.run("difficulty", difficultyEvent, hookKwargs, cache));

        if (lockFlag) {
            applyFlag(lockFlag, newItem, character);
        }

        const afterEquipEvent: Events.AfterItemEquip = {
            character,
            name,
            oldItem,
            newItem,
        ...
        wheelCallbacks.run("afterItemEquip", afterEquipEvent, hookKwargs, cache);
    }

    const afterOutfitEvent: Events.AfterOutfitEquip = {
        character,
        name,
    };
    wheelCallbacks.run("afterOutfitEquip", afterOutfitEvent, hookKwargs, cache);
